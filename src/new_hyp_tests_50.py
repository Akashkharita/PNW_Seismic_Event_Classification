...
from common_processing_functions import apply_cosine_taper, butterworth_filter
...
def extract_features(data, t, cfg_file, fs):
    ...
    for i in tqdm(range(len(data))):
        try:
            ...
            print(f"Error processing feature extraction: {e}")
            pass

    return df


def compute_features(win_before=2000, win_after=3000, nos=300, fmin=1, fmax=10, fs=100, tp=10, nc=4):
    ...
    with Pool(cpu_count()) as pool:
        df_eq = pool.apply(extract_features, (norm_eq, t_eq, cfg_file, fs))
        df_exp = pool.apply(extract_features, (norm_exp, t_exp, cfg_file, fs))
        df_su = pool.apply(extract_features, (norm_su, t_su, cfg_file, fs))
        df_no = pool.apply(extract_features, (norm_no, t_no, cfg_file, fs))
