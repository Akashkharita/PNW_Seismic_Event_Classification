...
for i in tqdm(range(len(buck_exp))):
     data_exp.append(f['/data/'+buck_exp[i]][ind_exp[i], 2, 4000:8000])
...
filtered_exp = np.array(butterworth_filter(data_exp, 1, 15, 100, 4, 'bandpass'))
...
# Resampling the data
exp_Z = np.array([resample_array(arr, 100, 50) for arr in exp_Z])
...
for i in tqdm(range(len(exp_Z))):
    try:
        df = time_series_features_extractor(cfg_file, exp_Z[i], fs= 50, verbose = 0)
        df['serial_no'] = i
...
X.to_csv('/home/ak287/PNW_Seismic_Event_Classification/extracted_features/P_10_30_F_1_15_50/tsfel_features_comcat_z_explosion_10_30.csv')
