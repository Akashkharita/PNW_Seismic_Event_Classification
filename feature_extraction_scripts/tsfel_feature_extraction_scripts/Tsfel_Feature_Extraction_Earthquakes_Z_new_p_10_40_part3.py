...
data_eq = []
for i in tqdm(range(100000, len(buck_eq))):
     data_eq.append(f['/data/'+buck_eq[i]][ind_eq[i], 2, 4000:9000])
...
tapered_eq = apply_cosine_taper(data_eq, taper_percent = 10) 

              
filtered_eq = np.array(butterworth_filter(data_eq, 1, 10, 100, 4, 'bandpass'))
...
for i in range(len(eq_Z)):
    try:
    
        df = time_series_features_extractor(cfg_file, eq_Z[i], fs=100,)
        df['serial_no'] = i+100000
...
X.to_csv('/home/ak287/Data_Mining_in_the_PNW/Extracted_Features/Tsfel_Features/tsfel_features_comcat_z_earthquake_10_40_part3.csv')
