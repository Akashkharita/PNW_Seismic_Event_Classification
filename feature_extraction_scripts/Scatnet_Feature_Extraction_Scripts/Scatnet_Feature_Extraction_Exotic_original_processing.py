...
def extract_waveforms(cat, file_name):
    ...
    for i in range(len(cat_trace)):
        ...
        if np.sum(f['/data/'+bucket][ind, 0, :18000]) != 0:
...
exotic_id = cat_exotic['event_id'].values
st_exotic = extract_waveforms(cat_exotic, exotic_file_name)

fs = 100  # Sampling frequency in Hz
cutoff = 1  # Cutoff frequency in Hz

data = np.array(st_exotic)[:,:,2000:17000]
...
np.savez(
    "../extracted_features/exotic_scattering_coefficients.npz",
