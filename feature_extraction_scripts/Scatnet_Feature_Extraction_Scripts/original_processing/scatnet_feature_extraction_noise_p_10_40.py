...
st_noise, serial_no = extract_waveforms(cat_noise, noise_file_name)

fs = 100  # Sampling frequency in Hz
cutoff = 1  # Cutoff frequency in Hz

data = np.array(st_noise)[:,:,4000:9000]
...
endtime = time.time()
dur_scatnet_noise = endtime - starttime
        
        
       
np.savez(
    "/home/ak287/PNW_ML_Classification/extracted_features/scatnet_features/p_10_40/noise_scattering_coefficients.npz",
    order_1=scattering_coefficients[0],
    order_2=scattering_coefficients[1],
    event_ids = serial_no,
    duration = dur_scatnet_noise
)
