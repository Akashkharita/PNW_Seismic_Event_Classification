...
def extract_waveforms(cat, file_name):
    ...
    for i in range(100000, 150000):
        ...
        if np.sum(f['/data/'+bucket][ind, 0, :15000]) != 0:
            ...
            comcat_id.append(cat_comcat['event_id'].values[i])
            serial_no.append(i)
...
data = np.array(st_comcat)[:,:,4000:9000]
...
endtime = time.time()
dur_scatnet_comcat = endtime - starttime
        
 
       
np.savez(
    "/home/ak287/PNW_ML_Classification/extracted_features/scatnet_features/p_10_40/comcat_scattering_coefficients_part3.npz",
