...
for i in tqdm(range(len(exp_Z))):
    try:
        df =   seis_feature.FeatureCalculator(exp_Z[i], fs = new_sr).compute_features()  #seis_feature.compute_physical_features(tr=tr, envfilter=False)
        df['serial_no'] = i
...
X.to_csv('/home/ak287/PNW_Seismic_Event_Classification/extracted_features/physical_features_explosion_P_'+str(start_time)+'_'+str(end_time)+'_F_'+str(int(low))+'_'+str(int(high))+'_'+str(new_sr)+'.csv')