...
for i in tqdm(range(len(buck_no))):
     data_no.append(f['/data/'+buck_no[i]][ind_no[i], 2, :])
...
no_df_Z = pd.DataFrame([])
for i in tqdm(range(len(no_Z))): 
    try:
        df = seis_feature.FeatureCalculator(no_Z[i]).compute_features()
        df['serial_no'] = i
        no_df_Z = pd.concat([no_df_Z, df])
    except:
        pass
...
X['source'] = y
 ...
 #X.to_csv('/home/ak287/Data_Mining_in_the_PNW/Extracted_Features/Physical_Features/physical_features_noise_z_new.csv')
X.to_csv('/home/ak287/PNW_Seismic_Event_Classification/extracted_features/new_physical_features_noise_z_50_100.csv')
