...
for i in tqdm(range(len(eq_Z))):
    try:
        ...
        df = seis_feature.FeatureCalculator(eq_Z[i], fs = new_sr).compute_features()
...
X.to_csv('/home/ak287/PNW_Seismic_Event_Classification/extracted_features/physical_features_earthquake_part_'+str(int(part))+'_P_'+str(start_time)+'_'+str(end_time)+'_F_'+str(int(low))+'_'+str(int(high))+'_'+str(new_sr)+'.csv')
