...
# python3 Exotic_Feature_Extraction.py --low 1 --high 15 --new_sr 50 --start_time 10 --end_time 30 --taper_amount 10 --original_sr 100 --num_corners 4
...
for i in tqdm(range(len(su_Z))):
    try:
        df = seis_feature.FeatureCalculator(su_Z[i], fs = new_sr).compute_features()
        df['serial_no'] = i
...
for i in tqdm(range(len(th_Z))):
    ...
    try:
        df = seis_feature.FeatureCalculator(th_Z[i], fs = new_sr).compute_features()
        df['serial_no'] = i
...
for i in tqdm(range(len(sb_Z))):
    try:
        df = seis_feature.FeatureCalculator(sb_Z[i], fs = new_sr).compute_features()
        df['serial_no'] = i
