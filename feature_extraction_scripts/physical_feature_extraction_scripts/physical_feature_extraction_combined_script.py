...
if low == 1.0:
    output_file = os.path.join(output_path, f'physical_features_{class_type}_P_{start_time}_{end_time}_F_{str(int(low)).replace(".","")}_{int(high)}_{new_sr}_part_{part}.csv')

else:
    output_file = os.path.join(output_path, f'physical_features_{class_type}_P_{start_time}_{end_time}_F_{str(low).replace(".","")}_{int(high)}_{new_sr}_part_{part}.csv')
